...
export default class FantasyCalendarView extends ItemView {
    ...
    triggerHelperEvent(event: string, full: boolean = true) {
        ...
        if (full) {
            this.updateMe = false;
            this.plugin.app.workspace.trigger("obsicals-updated");
        }
    }
...
class ChangeDateModal extends Modal {
    ...
    buildDateFields() {
        ...
        const dayEl = this.dateFieldEl.createDiv("obsical-date-field");
...
class ViewEventModal extends Modal {
    constructor(public event: Event, public plugin: FantasyCalendar) {
        super(plugin.app);
        this.containerEl.addClass("obsical-view-event");
    }
    async display() {
        this.contentEl.empty();
